...
| INT16 quantization | `quantization/quant.c/h` | Done — engine-wired | `fe_quantize_int16`, `fe_matmul_int16_dyn`, `fe_linear_int16` plus the static-scale pair `fe_matmul_int16_static`/`fe_linear_int16_static`; `fe_quantize_model_int16` is the `fe_quantize_model` analog (per-channel in-place repack), and `runtime/kernels.c` branches on `DTYPE_INT16` weights at run time |
| FP16 storage | `core/fp16.c/h` | Done — engine-wired | `DTYPE_FLOAT16` (2 bytes), `fe_f32_to_fp16`/`fe_fp16_to_f32` + bulk bufs, bit-exact round-trip test; `fe_repack_model_half` repacks weights in place and `runtime/kernels.c` upconverts on read, so compute stays FP32 |
| BF16 storage | `core/fp16.c/h` | Done — engine-wired | `DTYPE_BFLOAT16` (6th dtype, 2 bytes), `fe_f32_to_bf16`/`fe_bf16_to_f32` + bulk bufs (RNE on low 16 bits); pinned round-trip test; same repack + upconvert-on-read path as FP16 |
| Calibration | `runtime/engine.c`, `quantization/quant.c` | Done — collection + static scales | `fe_runtime_calibrate` runs a sample set through `fe_runtime_run`, recording per non-weight float tensor the observed `max(\|x\|)` across samples. `fe_quantize_model_static` turns those ranges into per-tensor `act_scale` values on the registry, and the engine routes MATMUL/LINEAR nodes whose activation input has a scale to the static INT8/INT16 kernels (unsampled tensors stay dynamic). Percentile/stat smoothing of the ranges remains |
...
| CI | `.github/workflows/ci.yml` | 11/11 jobs green on `main` | The eight core legs — test (matrixed over gcc and clang) / sanitize (mandatory ASan+UBSan) / fuzz (libFuzzer, corpus-seeded, bridge TU + clang probe fixed) / perf (Release `bench_avx2` vs a per-CPU baseline under `bench/`, seeded from the Ubuntu runner; pinned scalar references — matmul 4.78x, elementwise 3.92x, gemm 15.65x; 30% threshold, artifact upload re-seeds the baseline) / coverage (gcovr `--fail-under-line 70`) / pi (ARMv6 cross + QEMU ctest) / esp (IDF container compile gate: `optim/shape_infer.c` ships on device, `FERRITE_ROOT` resolution, `esp_timer` via REQUIRES) / golden (`tools/golden_gen.py` → `tools/golden_compare.py` runs the zoo through ONNX Runtime and Ferrite, tolerance-matched; runnable on the dev host with `onnxruntime`+`onnx` pip packages) — plus a Windows/MinGW leg and a Release leg built with `-Werror`. Baselines regenerated and observed on the Ubuntu runners |
...
- INT16/FP16/BF16 are engine-wired: `fe_quantize_model_int16` and `fe_repack_model_half` do the model repack, and `runtime/kernels.c` branches on the weight dtype (INT16 kernels; FP16/BF16 upconvert-on-read). Calibration feeds `fe_quantize_model_static`, so static activation scales are consumed by the engine. The open item is percentile/stat smoothing of the collected ranges — they are raw `max(|x|)` today.
...
- Ports: no NEON matmul (needs a Pi Zero 2 W+ target); ESP32 hardware validation is a documented manual step; CI is green end-to-end on `main` (all 11 jobs), with per-CPU baselines under `bench/` reseeded from the Linux Release runners.
...
4. Clean the repo: remove build artifacts, finalize `.gitignore`, move working docs under `docs/`.
...
9. ~~**Persistent/tiling improvements**~~ (Stage 8): `bench_avx2` extended with elementwise + GEMM transposes; Debug and Release numbers recorded in `docs/benchmarks.md`. **Done** — remainders still fall back to scalar (documented).
...
11. Route dispatch to `fe_matmul_avx2` behind `fe_cpu_has_avx2()`; build the benchmark suite. *(dispatch already wired; see `docs/benchmarks.md`)*
