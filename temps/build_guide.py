#!/usr/bin/env python3
"""
build_guide.py -- generate and compile the Ferrite technical guide PDF.

Generates ferrite_guide.tex (full LaTeX source, TikZ diagrams, code
listings) from structured content defined in this file, then compiles
it with tectonic (preferred) or pdflatex/xelatex.

Usage:  python build_guide.py
Output: ferrite_guide.tex + Ferrite_Guide.pdf next to this script.
"""
...
TEX_PATH = os.path.join(HERE, "ferrite_guide.tex")
PDF_PATH = os.path.join(HERE, "Ferrite_Guide.pdf")

# --------------------------------------------------------------------------
# Content data (kept in Python so tables stay in sync with the repo)
# --------------------------------------------------------------------------

DTYPES = [
    ("DTYPE_FLOAT32", "0", "4", "every kernel; the only dtype kernels accept today"),
    ("DTYPE_INT8", "1", "1", "quantization storage (symmetric per-tensor)"),
    ("DTYPE_INT32", "2", "4", "int8 GEMM accumulation target"),
]

STATUS_CODES = [
    ("FE_OK", "0", "full success; all outputs written"),
    ("FE_ERR_NULL", "1", "required pointer argument was NULL"),
    ("FE_ERR_SHAPE", "2", "shape/rank mismatch or unsupported shape; also the engine's ``unimplemented op'' signal"),
    ("FE_ERR_DTYPE", "3", "dtype mismatch, unknown, or unsupported"),
    ("FE_ERR_NOMEM", "4", "allocation failed"),
    ("FE_ERR_BOUNDS", "5", "index out of range"),
    ("FE_ERR_IO", "6", "file open/read/write/format failure (added for serialization)"),
]

OP_ENUM = [
    ("FE_OP_INPUT", "0", "graph input marker; runtime binds the caller's buffer here"),
    ("FE_OP_OUTPUT", "1", "graph output marker; runtime copies result out"),
    ("FE_OP_MATMUL", "2", "C = A x B, dispatches to AVX2 when available"),
    ("FE_OP_LINEAR", "3", "C = A x W + b (matmul then in-place bias)"),
    ("FE_OP_RELU", "4", "elementwise max(x, 0), in-place legal"),
    ("FE_OP_SOFTMAX", "5", "last-axis softmax with max-subtraction"),
    ("FE_OP_CONV1D", "6", "im2col + matmul; scratch malloc exception"),
    ("FE_OP_BATCHNORM", "7", "enum exists, kernel missing -> engine default arm"),
    ("FE_OP_ADD", "8", "rides fe_bias_add's column broadcast"),
    ("FE_OP_FLATTEN", "9", "{n, ...} -> {n, rest} view + copy"),
]
...
ONNX_OPS = [
    ...
    ("Conv", "CONV1D", "pads[0..1], strides[0], kernel_shape[0] parsed"),
...
WIRE_FIELDS = [
    ...
    ("NodeProto", "4", "op_type", "string mapped through the op table"),
    ("NodeProto", "5", "attribute", "sub-message: field 1 name, 7 ints (cap 8), 8 i"),
    ("TensorProto", "1", "dims", "repeated int64; >8 dims discarded"),
    ("TensorProto", "9", "raw_data", "raw little-endian float32 payload"),
]

TARGETS = [
    ("test_tensor", "strided ops, reshape/transpose/contiguous repack; slice views; stride-0 broadcast {1,3}->{4,3}; allclose tolerances"),
    ("test_allocator", "arena alignment math, reset identity, exhaustion, arena tensors"),
    ("test_ops", "matmul identity + known [[1,2],[3,4]]@[[5,6],[7,8]]=[[19,22],[43,50]]; relu; softmax sums to 1, stable on {1000,1e3,1002}; elementwise add/mul/div exact; mul_scalar/neg; axis sum/max on {2,3}; dot=32; logsumexp in (1000,1003)"),
    ("test_graph", "linear-chain topo order; diamond graph fan-out / merge"),
    ("test_engine", "1-4-8-3 MLP, uniform 0.1 weights => every softmax output exactly 1/3"),
    ("test_onnx", "loads tests/tiny_mlp.onnx; topo valid; weight arena non-empty"),
    ("test_planner", "reuse beats naive byte total; fe_plan_apply pointer placement"),
    ("test_conv1d", "edge detector {1,0,-1}, multichannel all-ones, pad=1 sum filter"),
    ("test_profiler", "MLP under profiler, 10 runs, sorted report printed"),
    ("test_quant", "scale == 1/127, endpoints +-127, dequant < 0.05, matmul err < 0.1, exactly 75% smaller"),
    ("test_tensor_ser", "round-trips (all dtypes, up to 8-D), view saves canonical, 8 corruption cases"),
]

FILES_TABLE = [
    ("core/types.h", "FeDtype, FeStatus enums, fe_dtype_size, FERRITE_MAX_DIMS=8"),
    ("core/tensor.c/h", "FeTensor strided views: alloc/slice/transpose/reshape/contiguous/copy; stride-0 broadcast_to; numel; get/set_f32; allclose"),
    ("core/tensor_ser.c/h", "versioned binary serialization (FETN v1), strict validating reader"),
    ("core/allocator.c/h", "FeArena bump allocator; pointer-address alignment; peak tracking"),
    ("core/log.c/h", "fe_log level-filtered stderr logging (compile-time minimum)"),
    ("graph/graph.c/h", "fixed-capacity IR: nodes, tensor registry, Kahn topological sort"),
    ("ops/matmul.c", "i-k-j scalar reference + AVX2 dispatch behind CPUID gate; fe_linear wrapper"),
    ("ops/activations.c", "relu, last-axis stable softmax (inv-sum third pass), broadcast bias_add"),
    ("ops/elementwise.c", "fe_add/sub/mul/div + scalar variants + fe_neg; exact-shape fp32 flat loops"),
    ("ops/reduce.c", "single-axis fe_sum/fe_max/fe_min with axis-removed output; rank-1 fe_dot"),
    ("ops/stability.c", "max-shift fe_logsumexp and fe_stable_exp_normalize (1-D fp32, two passes)"),
    ("ops/conv1d.c", "im2col transformation + matmul; per-call scratch buffer"),
    ("simd/matmul_avx2.c/h", "tiled AVX2+FMA microkernel; CPUID detection; remainder columns"),
    ("planner/memory_planner.c/h", "lifetime analysis + greedy best-fit buffer reuse; FePlan offsets"),
    ...
    ("tools/profiler.c", "per-op ns timing records, CLOCK_MONOTONIC, sorted report"),
    ("tools/bench_matmul.c", "naive GFLOPS at N=256 over 20 runs"),
    ("tools/bench_avx2.c", "AVX2 vs naive GFLOPS, correctness verify, speedup print"),
    ("tools/demo_acousticleaknet.c", "end-to-end demo: acoustic leak-detection CNN on synthetic audio"),
]
...
def rows(table, cols=None):
    if cols:
        table = [[r[c] for c in cols] for r in table]
    return "\\\\\n".join(" & ".join(esc(c) for c in r) for r in table) + "\\\\"
...
# Build files table programmatically
_files_rows = []
for f, role in FILES_TABLE:
    _files_rows.append(f"{esc(f)} & {esc(role)}")
FILE_TABLE_LATEX = r"""
...
\midrule
""" + rows([tuple(r) for r in DTYPES]) + r"""
\bottomrule
\end{tabular}
...
BODY = (
    PREAMBLE + INTRO + FILE_TABLE_LATEX.replace("\\textwidth", "0.94\\textwidth")
    + CORE_TENSOR.replace(
        "\\subsection{The Arena Allocator --- \\texttt{FeArena}}",
        TENSOR_VIEWS + "\\subsection{The Arena Allocator --- \\texttt{FeArena}}",
    )
    + SERIALIZATION + GRAPH
    + KERNELS.replace(
        "\\subsection{Conv1d via im2col}",
        ELEMENTWISE + REDUCE_SUB + STABILITY_SUB + "\\subsection{Conv1d via im2col}",
    )
    + SIMD + PLANNER + ENGINE + ONNX + QUANT + TOOLS + BUILD_TESTS + FLOW_HONEST
)
...
def find_engine():
    """Locate a LaTeX engine: tectonic preferred, then pdflatex."""
    exe = shutil.which("tectonic") or os.environ.get("TECTONIC_BIN")
    candidates = [exe] if exe else []
    # CLion-independent common spots
    local = os.path.expandvars(r"%LOCALAPPDATA%\Programs")
    for pat in (os.path.join(local, "tectonic", "tectonic.exe"),):
    ...
    if pdflatex:
        return pdflatex, ["pdflatex", "-interaction=nonstopmode"] * 1
    return None, None


def compile_pdf(engine, args_mode):
    """Compile; returns combined compiler output (contains 'N pages')."""
    if args_mode == ["tectonic"]:
        cmd = [engine, "-X", "compile", TEX_PATH]
    else:
        cmd = [engine, "-interaction=nonstopmode", "-halt-on-error",
               os.path.basename(TEX_PATH)]
    for attempt in range(2):  # twice for TOC/refs (tectonic does passes itself)
        p = subprocess.run(cmd, cwd=HERE, capture_output=True, text=True,
                           encoding="utf-8", errors="replace", timeout=900)
        log = p.stdout + p.stderr
        if p.returncode != 0:
            print(log[-4000:] if len(log) > 4000 else log)
            raise SystemExit(f"LaTeX failed (exit {p.returncode})")
        if args_mode == ["tectonic"]:
            break  # tectonic reruns internal passes automatically
    return log


def count_pages(log):
    # 1) proper parser when available
    try:
        from pypdf import PdfReader
        return len(PdfReader(PDF_PATH).pages)
    except ImportError:
        pass
    # 2) inflate compressed object streams and count page objects
    try:
        import zlib
        ...
        pass
    # 3) last resort: the engine's own report
...
def main():
    ...
    engine, mode = find_engine()
    if not engine:
        raise SystemExit(
            "No LaTeX engine found. Install tectonic "
            "(winget install Tectonic.Tectonic) or ensure pdflatex is on PATH."
        )
    print(f"compiling with {os.path.basename(engine)} ...")
    log = compile_pdf(engine, mode)

    pages = count_pages(log)
    size_kb = os.path.getsize(PDF_PATH) // 1024
    print(f"OK: {PDF_PATH}  pages={pages}  size={size_kb} KB")
    if pages < 5:
        raise SystemExit(f"FAIL: only {pages} pages (< 5 required)")
