...
| CI | `.github/workflows/ci.yml` | Authored, CI-only | test / sanitize (mandatory ASan+UBSan) / fuzz (libFuzzer, corpus-seeded) / perf (`bench_avx2` vs `temps/bench_baseline.json`, 20% threshold) / coverage (gcovr `--fail-under-line 70`) / pi (ARMv6 cross + QEMU ctest) / esp (IDF container compile gate) — unverifiable on the dev host; see `CHANGELOG.md` |
